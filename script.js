// --- DATA STORE ---
// A comprehensive object containing  all curated options.
    ...
    ['model', 'product', 'film'].forEach(m => {
        ...
        if (PROMPT_OPTIONS[m]) {
            PROMPT_OPTIONS[m].fields.forEach(f => {
                initialFormState[m][f] = { select: PROMPT_OPTIONS[m]['conservative']?.[f]?.[0] || '', custom: '' };
            });
        }
    });
...
// --- DYNAMIC HTML COMPONENT BUILDERS ---
...
function FormField({ id, label, options, value, customValue, isLocked = false }) {
    return `
        <div class="mb-6">
            ...
            <select id="${id}-select" class="form-select w-full p-3 bg-gray-100 border border-gray-300 rounded-lg text-gray-800">
            ...
            <input type="text" id="${id}-text" value="${customValue}" placeholder="Custom text (optional)" class="form-custom-text w-full p-3 bg-gray-100 border border-gray-300 rounded-lg text-gray-800 mt-2">
        </div>
    `;
}
...
function renderApp() {
    ...
    const { mode, intensity, formState, isLoading, outputs } = state;
    ...
    const appHTML = `
        <div class="w-full max-w-4xl mx-auto p-4 sm:p-6">
            ...
            ${outputs ? `<section class="mt-8 bg-white rounded-2xl shadow-xl p-6 sm:p-10">${OutputSection({ title: "Generated Text Prompt", content: outputs.text })}</section>` : ''}
        </div>
    `;
...
function addEventListeners() {
    document.getElementById('mode-selector')?.addEventListener('click', e => {
        if (e.target.dataset.value && state.mode !== e.target.dataset.value) {
    ...
    document.getElementById('intensity-selector')?.addEventListener('click', e => {
        if (e.target.dataset.value && state.intensity !== e.target.dataset.value) {
            state.intensity = e.target.dataset.value;
            renderApp();
        }
    });
...
function handleFormChange(e) {
    ...
    if (state.formState[mode] && state.formState[mode][fieldId]) {
        if (target.classList.contains('form-select')) {
            state.formState[mode][fieldId].select = target.value;
            state.formState[mode][fieldId].custom = '';
            document.getElementById(`${id}-text`).value = '';
        } else {
            state.formState[mode][fieldId].custom = target.value;
            document.getElementById(`${id}-select`).value = '';
        }
    }
}
...
async function handleSubmit() {
    ...
    const data = { mode: state.mode, intensity: state.intensity };
    PROMPT_OPTIONS[mode].fields.forEach(field => {
        const fieldState = formState[mode][field];
        data[field] = fieldState.custom.trim() || fieldState.select;
    });
    ...
    const finalPrompt = `You are a senior art director. Synthesize the following creative parameters into a single, concise, and punchy paragraph for an image generation AI: ${parameterString}.`;
    
    const generatedText = await callGeminiAPI(finalPrompt);
    
    if (generatedText) {
        state.outputs = { text: generatedText.trim() };
    }
